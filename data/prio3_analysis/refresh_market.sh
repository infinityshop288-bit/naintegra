...
echo "[1/12] macro..."         && $PY macro.py            >/dev/null && echo "  ok"
echo "[2/12] analise..."       && $PY multi_analysis.py   >/dev/null && echo "  ok"
echo "[3/12] opcoes (COTAHIST)..." && $PY build_opts.py   >/dev/null && echo "  ok"
echo "[4/12] opcoes (agrega)..."   && $PY multi_options.py >/dev/null && echo "  ok"
echo "[5/12] fatos/result..."  && $PY scrape_fatos_multi.py >/dev/null && echo "  ok"
echo "[6/12] macro+juros..."   && $PY macro.py            >/dev/null && echo "  ok"
echo "[7/12] fundamentos..."   && $PY fundamentals_multi.py >/dev/null && echo "  ok"
