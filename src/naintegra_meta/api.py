...
from naintegra_meta.marketing_data import AUTOMATION_HYPOTHESES, COMPETITORS, POSITIONING
...
def content_ideas(
    ...
    del user
    try:
        ideas, source = generate_content_ideas(body.tema, body.formato, settings=settings)
...
def competitors(user: AuthUser = Depends(require_delegado_user)) -> dict[str, Any]:
    del user
    return {"competitors": COMPETITORS, "positioning": POSITIONING}
