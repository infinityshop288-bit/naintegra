/** Modal de autenticação — login, cadastro, recuperação, Google e Apple. */
(function () {
  ...
  function renderTopbarUser(user) {
    ...
    if (user) {
      ...
      slot.innerHTML = `<button type="button" class="btn sm primary" id="auth-open">Entrar</button>`;
  ...
  function bind(view) {
    ...
    document.querySelectorAll("[data-auth-view]").forEach((btn) => {
      btn.addEventListener("click", () => open(btn.getAttribute("data-auth-view")));
    });
    document.querySelectorAll("[data-oauth]").forEach((btn) => {
      btn.addEventListener("click", async () => {
        try {
          ...
          open(view, err.message || "Não foi possível iniciar login social.");
        }
      });
    });
    ...
    form?.addEventListener("submit", async (e) => {
      ...
      try {
        if (view === "login") {
          ...
          open("login", "Conta criada. Confirme seu e-mail se solicitado e faça login.");
        } else if (view === "recover") {
          await window.LexAuth.resetPassword(email);
          open("login", "Enviamos um link de recuperação para seu e-mail.");
        ...
        const msgEl = form.querySelector(".auth-error") || document.createElement("p");
        msgEl.className = "auth-error";
        msgEl.textContent = err.message || "Erro ao autenticar.";
        if (!msgEl.parentElement) form.prepend(msgEl);
      } finally {
